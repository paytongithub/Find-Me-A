...
app.MapGet("/details", async (string title) =>
{
   ...
   if (t != null)
   {
       ...
       sb.Append($"<button class=\"dt-watchlist-btn\" onclick='addToWatchlist({System.Text.Json.JsonSerializer.Serialize(t.TitleName)}, {System.Text.Json.JsonSerializer.Serialize(t.TitleType)})'>+ Add to Watchlist</button>\n");
   ...
   sb.Append(@"<script>
async function addToWatchlist(title, titleType) {
 ...
 body.append('mediaType', (titleType || '').toLowerCase() === 'tv' ? 'tv' : 'movie');
 try {
...
app.MapPost("/watchlist/add", (string username, string movieTitle, string? rating, string? mediaType, int? tmdbId) =>
{
   if (string.IsNullOrEmpty(connectionString))
   {
       return Results.BadRequest(new { error = "DB_CONNECTION is missing" });
   ...
   {
       if (!int.TryParse(rating.Trim(), out parsedRating) || parsedRating < 0 || parsedRating > 10)
       {
           return Results.BadRequest(new { error = "Rating must be a whole number from 0 to 10." });
       }
   }
   ...
   try
   {
       ...
       watchList.AddToWatchList(username, movieTitle, DateTime.Now, parsedRating, mediaType, tmdbId);


       return Results.Ok(new { message = $"Successfully added {movieTitle} to watchlist." });
   }
   catch (Exception ex)
   {
...
app.MapGet("/watchlist", async (string username) =>
{
   ...
   try
   {
       ...
       foreach (var item in items)
       {
           ...
           try
           {
               poster = await tmdb.GetPosterPathByTitleName(item.Title.TitleName, item.Title.TitleType);
           }
           catch
           {
               poster = null;
