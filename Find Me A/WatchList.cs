...
namespace Find_Me_A
{
   public class WatchList
   {
       ...
       public void AddToWatchList(string Username, string TitleName, DateTime WatchedDate, string? mediaType = null, int? tmdbId = null)
       {
           using (SqlConnection conn = new SqlConnection(_connectionString))
           {
               ...
               int TitleID = EnsureTitleExists(conn, TitleName, mediaType, tmdbId);
       ...
       public void AddToWatchList(string Username, string TitleName, DateTime WatchedDate, int UserRating, string? mediaType = null, int? tmdbId = null)
       {
           using (SqlConnection conn = new SqlConnection(_connectionString))
           {
               conn.Open();
       ...
       // Try get title id by name and type without throwing
       private int? TryGetTitleId(SqlConnection conn, string TitleName, string? titleType)
       {
           if (string.IsNullOrWhiteSpace(titleType))
               return TryGetTitleId(conn, TitleName);


           using (SqlCommand cmd = new SqlCommand(
               "SELECT TitleID FROM Titles WHERE TitleName = @TitleName AND TitleType = @TitleType", conn))
           {
               cmd.Parameters.AddWithValue("@TitleName", TitleName);
               cmd.Parameters.AddWithValue("@TitleType", NormalizeTitleType(titleType));
               object result = cmd.ExecuteScalar();
               if (result == null) return null;
               return Convert.ToInt32(result);
           }
       ...
       private static string NormalizeTitleType(string? mediaType)
       {
           var normalized = mediaType?.Trim();
           return string.Equals(normalized, "tv", StringComparison.OrdinalIgnoreCase) ||
       ...
       private int EnsureTitleExists(SqlConnection conn, string titleName, string? mediaType = null, int? tmdbId = null)
       {
           // check existing
           var existing = TryGetTitleId(conn, titleName, mediaType);
           ...
           try
           {
               if (tmdbId.HasValue && !string.IsNullOrWhiteSpace(mediaType))
               {
                   tmdbTitle = tmdb.GetTitleById(tmdbId.Value, mediaType).GetAwaiter().GetResult();
               }
               else
               {
           ...
           using (var tran = conn.BeginTransaction())
           {
               try
               {
                       ...
                       SELECT CAST(SCOPE_IDENTITY() AS INT);", conn, tran))
                   {
                       ...
                       cmd.Parameters.AddWithValue("@TitleType", NormalizeTitleType(tmdbTitle.TitleType));
