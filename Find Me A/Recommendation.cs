            ...
            using (var conn = new SqlConnection(connectionString))
            {
                ...
                string sql = @"
                    SELECT
                        ...
                        uwt.WatchedDate,
                        uwt.UserRating,
                        (SELECT STRING_AGG(CAST(a2.ActorID AS NVARCHAR(20)), ', ') FROM (SELECT TOP (20) a2.ActorID FROM TitleActors ta2 JOIN Actors a2 ON ta2.ActorID = a2.ActorID WHERE ta2.TitleID = t.TitleID ORDER BY a2.ActorID) a) AS ActorIds,
                        (SELECT STRING_AGG(CAST(g2.GenreID AS NVARCHAR(20)), ', ') FROM (SELECT TOP (5) g2.GenreID FROM TitleGenres tg2 JOIN Genres g2 ON tg2.GenreID = g2.GenreID WHERE tg2.TitleID = t.TitleID ORDER BY g2.GenreID) g) AS GenreIds
